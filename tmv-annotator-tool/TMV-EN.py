         ...
         for i in dep_dict[dep_pos]:
            ...
            if (curr_pos in verbal_tags):
               ...
               if i not in fin_pos:
                  ...
                  elif curr_pos in ["RP", "CC"] or (curr_pos in ["VBN", "VB"] and curr_rel in ["VC", "IM", "CONJ", "PRD"]):
                     if curr_rel == "PRD":
                        print "Predicative copula construction!"
   ...
   for fp in fin_pos:
      ...
      if curr_pos == "TO":
         ...
         if checkInf(dep_dict[fp], dep_dict, pos_dict): 
            res[str(fp)+"#"+curr_pos+"#"+curr_rel+"#"+curr_morph+"#"+curr_token+"#"+curr_lemma+"#"] = createDepDict(fp, fin_pos, last_fin_pos, inf_vcs, coord)
      else:
         last_fin_pos = fp
         res[str(fp)+"#"+curr_pos+"#"+curr_rel+"#"+curr_morph+"#"+curr_token+"#"+curr_lemma+"#"] = createDepDict(fp, fin_pos, last_fin_pos, inf_vcs, coord)
...
def getTenseEN(chain_dict):
   ...
   def deriveTMEN(pos_seq, fin, chain_dict):
      ...
      vp_properties = {'mod': False, 'fut_mod': False, 'subj_mod': False, 'finV': 0, 'aux_pres': False, 'aux_past': False, 'aux_part': False, 'aux_ger': False, 'aux_be_pres': False, 'aux_be_ger': False, 'aux_be_past': False, 'aux_inf': False, 'part': False, 'inf': False, 'ger': False, 'aux_do_pres': False, 'aux_do_past': False, 'aux_be_pass': False, 'to': False, 'main': "err"}
   ...
   for fin in chain_dict:
      ...
      temp_res = finite + "\t" + str(mainV) + "\t" + tense + "\t" + mood + "\t" + voice + "\t" + progressive + "\t" + negation
      res.append(temp_res)
...
def extractVerbDeps(parsed_file):
    ...
    while parsed_line:
        ...
        print "Deriving TMV for English ...", sent_nr, "\r",
